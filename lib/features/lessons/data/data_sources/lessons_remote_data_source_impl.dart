...
/// the full rationale.
///
/// The lessons list is paginated server-side (`{data, pagination}`) — see
...
class LessonsRemoteDataSourceImpl implements LessonsRemoteDataSource {
  ...
  @override
  Future<PaginatedResult<LessonModel>> getLessons(String chapterId, {int page = 1, int limit = 50}) async {
    try {
      final Response<Map<String, dynamic>> response = await _dio.get(
        Endpoints.lessons(chapterId),
        queryParameters: {'page': page, 'limit': limit},
      );
      final List<dynamic> list = (response.data?['data'] as List<dynamic>?) ?? [];
      final List<LessonModel> items = list.map((e) => LessonModel.fromJson(e as Map<String, dynamic>)).toList();
